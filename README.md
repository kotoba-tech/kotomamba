...
[scripts/](scripts/) contains scripts for training and inference mamba models.

[scripts/abci/inference](scripts/abci/inference) are inference scripts. `inference_test.sh` is a script for testing inference. (some modified official mamba inference script)

[scripts/abci/training](scripts/abci/training) are training scripts.
`A-node/` contains scripts for training on NVIDIA A100 GPUs and `V-node/` contains scripts for training on NVIDIA V100 GPUs.

[scripts/abci/training/V-node/mambda-130m.sh](scripts/abci/training/V-node/mambda-130m.sh) may be suitable for first training of mamba models.

### Tokenization

tokenization scripts are in [scripts/abci/data/](scripts/abci/data/).
